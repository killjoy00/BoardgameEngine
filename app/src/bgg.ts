...
export type BggThing = {
  ...
  expansionIds: number[];
  polls: Record<string, BggPollVotes>;
};
...
const decodeXml = (value: string) =>
  value
    ...
    .replace(/&gt;/g, ">")
    .replace(/&amp;/g, "&");
const numberValue = (body: string, tag: string) => {
  const raw = new RegExp(`<${tag}[^>]*value="([^"]*)"`).exec(body)?.[1];
  if (raw === undefined || raw === "") return null;
  const value = Number(raw);
  return Number.isFinite(value) ? value : null;
};
const textValue = (body: string, tag: string) => {
  const raw = new RegExp(`<${tag}\\b[^>]*>([\\s\\S]*?)<\\/${tag}>`).exec(body)?.[1];
...
export function parseThingXml(xml: string): BggThing[] {
  return [...xml.matchAll(/<item\b([^>]*)>([\s\S]*?)<\/item>/g)].map((match) => {
    ...
    const poll =
      /<poll[^>]*name="suggested_numplayers"[^>]*>([\s\S]*?)<\/poll>/.exec(body)?.[1] ?? "";
    for (const result of poll.matchAll(
      /<results[^>]*numplayers="([^"]+)"[^>]*>([\s\S]*?)<\/results>/g
    )) {
      const votes = (label: string) =>
        Number(
          new RegExp(`<result[^>]*value="${label}"[^>]*numvotes="(\\d+)"`).exec(result[2])?.[1] ?? 0
        );
      polls[result[1]] = {
        best: votes("Best"),
    ...
    const links = (type: string) =>
      [...body.matchAll(new RegExp(`<link[^>]*type="${type}"[^>]*>`, `g`))]
        .map((x) => ({ id: Number(attr(x[0], "id")), name: decodeXml(attr(x[0], "value")) }))
        .filter((x) => Number.isFinite(x.id));
    const expansionIds = links("boardgameexpansion").map((x) => x.id);
    return {
      id: Number(attr(head, "id")),
      type: attr(head, "type"),
      name: decodeXml(/<name[^>]*type="primary"[^>]*value="([^"]+)"/.exec(body)?.[1] ?? ""),
      year: numberValue(body, "yearpublished"),
      minPlayers: numberValue(body, "minplayers") ?? 0,
      maxPlayers: numberValue(body, "maxplayers") ?? 0,
      minutes: numberValue(body, "playingtime") ?? 0,
      weight: numberValue(body, "averageweight"),
      image: textValue(body, "image"),
      thumbnail: textValue(body, "thumbnail"),
...
export function parseCollectionXml(xml: string): BggCollectionItem[] {
  return [...xml.matchAll(/<item\b([^>]*)>([\s\S]*?)<\/item>/g)]
    .map((match) => {
      const head = match[1],
        body = match[2],
        status = /<status\b([^>]*)\/?>(?:<\/status>)?/.exec(body)?.[1] ?? "",
        ratingRaw = /<rating[^>]*value="([^"]*)"/.exec(body)?.[1] ?? "";
      return {
        id: Number(attr(head, "objectid")),
        collId: attr(head, "collid"),
        name: textValue(body, "name") ?? "",
        subtype: attr(head, "subtype"),
        own: attr(status, "own") === "1",
        forTrade: attr(status, "fortrade") === "1",
        wishlist: attr(status, "wishlist") === "1",
        wishlistPriority: (() => {
          const value = Number(attr(status, "wishlistpriority"));
...
export function parseSearchXml(xml: string): BggSearchResult[] {
  return [...xml.matchAll(/<item\b([^>]*)>([\s\S]*?)<\/item>/g)]
    .map((match) => ({
      ...
      name: decodeXml(/<name[^>]*value="([^"]*)"/.exec(match[2])?.[1] ?? ""),
      year: numberValue(match[2], "yearpublished")
    }))
    .filter((item) => Number.isFinite(item.id) && item.id > 0);
}

export function batches<T>(items: T[], size = 20): T[][] {
  if (size < 1 || size > 20) throw new Error("BGG batches must contain 1-20 items");
  const out: T[][] = [];
...
export class BggClient {
  ...
  constructor(
    ...
    if (!token) throw new Error("BGG API token is not configured");
  ...
  async collection(
    ...
    const query = new URLSearchParams({ username, ...toParams(params) });
    return parseCollectionXml(await this.request("collection", query));
  }
  async things(ids: number[]): Promise<BggThing[]> {
    if (!ids.length) return [];
    if (ids.length > 20) throw new Error("BGG Thing requests are limited to 20 IDs");
    return parseThingXml(
      await this.request("thing", new URLSearchParams({ id: ids.join(","), stats: "1" }))
    );
  }
  async search(query: string, exact = false): Promise<BggSearchResult[]> {
    ...
    return parseSearchXml(await this.request("search", params));
  }
...
function toParams(input: Record<string, string | number | boolean>): Record<string, string> {
