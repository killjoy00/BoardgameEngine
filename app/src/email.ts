  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      text: [
        ...
        "",
        "Opening this link will not consume it. Confirm sign-in on the page to continue.",
        input.url,
        "",
...
export async function sendInvitationEmail(input: InvitationEmail): Promise<void> {
  const response = await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: { Authorization: `Bearer ${input.apiKey}`, "Content-Type": "application/json" },
    body: JSON.stringify({
      from: input.from,
      to: [input.to],
      subject: "You’re invited to BoardGameEngine",
      text: `You’re invited to BoardGameEngine.\n\nSign in here: ${input.url}\n\nThis invitation was sent by the BoardGameEngine administrator.`
    })
  });
  if (!response.ok) throw new Error(`Resend rejected the invitation (${response.status})`);
}
