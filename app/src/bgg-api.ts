...
api.post("/sync/:id/enrich", async (c) => {
  const user = c.get("user"),
    ...
    run = await c.env.DB.prepare(
      "SELECT r.id,r.status,r.source_account_id sourceAccountId,sa.last_request_at lastRequestAt FROM bgg_sync_runs r JOIN source_accounts sa ON sa.id=r.source_account_id WHERE r.id=? AND r.user_id=?"
    )
      .bind(runId, user.id)
      .first<{
        id: string;
  ...
  if (!run) return c.json({ error: "Sync run not found" }, 404);
  ...
  if (!pending.results.length)
    return c.json({
      run: withDuration(await finalizeRun(c.env.DB, user.id, runId, run.sourceAccountId)),
      nextRequestAfterMs: 0
    });
  const retryAfterMs = await claimBggSlot(c.env.DB, run.sourceAccountId, run.lastRequestAt);
  if (retryAfterMs > 0)
    return c.json(
      { error: "BoardGameGeek requests are paced to protect the API", retryAfterMs },
      429
  ...
  let attempts = 0,
    ...
    things: BggThing[];
  try {
    things = await new BggClient(c.env.BGG_API_TOKEN, {
      onAttempt: (_status, attempt) => {
        attempts++;
        if (attempt > 1) retries++;
      }
    ...
    await c.env.DB.batch([
      ...
      ).bind(message, attempts, retries, runId, user.id),
      c.env.DB.prepare("UPDATE source_accounts SET last_error=?,updated_at=? WHERE id=?").bind(
        message,
  ...
  statements.push(
    ...
    ).bind(attempts, retries, omitted, runId, user.id)
  );
  for (const id of ids) {
    ...
    statements.push(c.env.DB.prepare("DELETE FROM game_polls WHERE bgg_id=?").bind(id));
    ...
    statements.push(c.env.DB.prepare("DELETE FROM game_tags WHERE bgg_id=?").bind(id));
    for (const tag of thing.categories)
      statements.push(
        c.env.DB.prepare(
          'INSERT INTO game_tags(bgg_id,kind,bgg_tag_id,name)VALUES(?,"category",?,?)'
        ).bind(id, tag.id, tag.name)
      );
    for (const tag of thing.mechanics)
      statements.push(
        c.env.DB.prepare(
          'INSERT INTO game_tags(bgg_id,kind,bgg_tag_id,name)VALUES(?,"mechanic",?,?)'
        ).bind(id, tag.id, tag.name)
      );
    statements.push(
      c.env.DB.prepare(
        "UPDATE bgg_sync_items SET status='done',error=NULL WHERE run_id=? AND bgg_id=?"
      ).bind(runId, id)
    );
  }
  for (const group of chunks(statements, 80)) if (group.length) await c.env.DB.batch(group);
  const status = await refreshRun(c.env.DB, user.id, runId, run.sourceAccountId);
  return c.json({
    run: withDuration(status),
