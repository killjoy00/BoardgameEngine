...
const resend = async (path, options = {}) => {
  ...
  if (!response.ok)
    throw new Error(`Resend ${path} failed (${response.status}): ${JSON.stringify(body)}`);
  return body;
};
...
if (!accounts.some((account) => account.id === accountId)) {
  if (accounts.length !== 1)
    throw new Error(
      "CLOUDFLARE_ACCOUNT_ID is not accessible and the token does not identify exactly one fallback account"
    );
...
await writeFile(
  configPath,
  config.replace("replace-after-running-wrangler-d1-create", database.uuid)
);
...
if (zones.length !== 1)
  throw new Error(`Expected one Cloudflare zone for ${zoneName}, found ${zones.length}`);
const zoneId = zones[0].id;

const siteRecords = await cf(
  `/zones/${zoneId}/dns_records?name=${encodeURIComponent(sendingDomain)}`
);
for (const siteRecord of siteRecords.filter(
...
for (const record of domain.records) {
  ...
  if (legacyName && legacyName !== name) {
    const legacy = await cf(
      `/zones/${zoneId}/dns_records?type=${record.type}&name=${encodeURIComponent(legacyName)}`
    );
    for (const obsolete of legacy)
      await cf(`/zones/${zoneId}/dns_records/${obsolete.id}`, { method: "DELETE" });
  }
}

await resend(`/domains/${domain.id}/verify`, { method: "POST" });
console.log(
  `Cloudflare D1 and Resend DNS are configured; verification requested for ${sendingDomain}.`
);
